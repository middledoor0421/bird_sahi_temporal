...
OUT_ROOT="${OUT_ROOT:-output/thesis2/stage1_sweeps/experiments}"
...
TILE_SCORE_MODE="mean"
TILE_APPLY_ROW_QUOTA="0"
TILE_QUOTA_MODE="top1"
TILE_USE_MEMORY="0"
TILE_MEMORY_TTL="0"
TILE_CAND_EXTRA="6"

case "${TAG}" in
  2026-03-12_exp1_posgate_relaxed)
    POS_MOTION_THR="4.8"
    POS_DIFF_RATIO_THR="1.2"
    ;;
  2026-03-12_exp2_tilemem_p95)
    TILE_SCORE_MODE="p95"
    TILE_APPLY_ROW_QUOTA="1"
    TILE_QUOTA_MODE="top1_bottom1"
    TILE_USE_MEMORY="1"
    TILE_MEMORY_TTL="3"
    ;;
  2026-03-12_exp3_hybrid_guarded)
    POS_MOTION_THR="4.8"
    POS_DIFF_RATIO_THR="1.2"
...
run_pair() {
  ...
  if [[ ! -f "${pred_path}" || ! -f "${stats_path}" ]]; then
    "${PYTHON_BIN}" scripts2/run_detector2.py \
      ...
      --empty-explore-rate 0.10 \
      --empty-explore-cap 2.0 \
      ...
      --tile-score-mode "${TILE_SCORE_MODE}" \
      --tile-apply-row-quota "${TILE_APPLY_ROW_QUOTA}" \
      --tile-quota-mode "${TILE_QUOTA_MODE}" \
      --tile-use-memory "${TILE_USE_MEMORY}" \
      --tile-memory-ttl "${TILE_MEMORY_TTL}" \
      --tile-cand-extra "${TILE_CAND_EXTRA}"
  fi
