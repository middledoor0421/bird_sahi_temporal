...
case "${TAG}" in
  2026-03-11_v7_noconfirm_fbdsv_val)
    run_pair "fbdsv" "${FBD_ROOT}" "val" "${TAG}" "0" "1" "1" "0" \
  ...
  2026-03-11_v7_nodual_fbdsv_val)
    run_pair "fbdsv" "${FBD_ROOT}" "val" "${TAG}" "1" "0" "1" "0" \
      "pred_slim_temporal_sahi_v7_exp0_val.jsonl.gz" "stats_temporal_sahi_v7_exp0_val.json" \
      "${FBD_ROOT}/annotations/fbdsv24_vid_val.json" "${FBD_ROOT}/annotations/fbdsv24_vid_val_sequences.json"
    ;;
  2026-03-11_v7_notargetverify_fbdsv_val)
    run_pair "fbdsv" "${FBD_ROOT}" "val" "${TAG}" "1" "1" "0" "0" \
      "pred_slim_temporal_sahi_v7_exp0_val.jsonl.gz" "stats_temporal_sahi_v7_exp0_val.json" \
      "${FBD_ROOT}/annotations/fbdsv24_vid_val.json" "${FBD_ROOT}/annotations/fbdsv24_vid_val_sequences.json"
    ;;
  2026-03-11_v7_noconfirm_icct_dev)
    run_pair "icct_sparse20" "${ICCT_ROOT}" "dev" "${TAG}" "0" "1" "1" "1" \
  ...
  2026-03-11_v7_nodual_icct_dev)
    run_pair "icct_sparse20" "${ICCT_ROOT}" "dev" "${TAG}" "1" "0" "1" "1" \
      "pred_slim_temporal_sahi_v7_exp0_dev.jsonl.gz" "stats_temporal_sahi_v7_exp0_dev.json" \
      "${ICCT_ROOT}/annotations/fbdsv24_vid_dev.json" "${ICCT_ROOT}/annotations/fbdsv24_vid_dev_sequences.json"
    ;;
  2026-03-11_v7_notargetverify_icct_dev)
    run_pair "icct_sparse20" "${ICCT_ROOT}" "dev" "${TAG}" "1" "1" "0" "1" \
      "pred_slim_temporal_sahi_v7_exp0_dev.jsonl.gz" "stats_temporal_sahi_v7_exp0_dev.json" \
      "${ICCT_ROOT}/annotations/fbdsv24_vid_dev.json" "${ICCT_ROOT}/annotations/fbdsv24_vid_dev_sequences.json"
    ;;
  *)
