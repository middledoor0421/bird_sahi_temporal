...
def main() -> None:
    ...
    if (cost_out_path == "" or cost_out_path == "/dev/null") and str(args.method) in [
        "temporal_sahi_v7",
        "yolo_sahi_always_verify",
        "yolo_sahi_keyframe_verify",
    ] and args.stats:
        inferred_cost = os.path.join(os.path.dirname(str(args.stats)), "cost_per_frame.jsonl.gz")
        if os.path.exists(inferred_cost):
            cost_out_path = inferred_cost
    ...
    verifier_backend = "sahi"
    if str(args.method) in V7_FAMILY_METHODS and isinstance(v7_run_meta, dict):
        verifier_backend = str(v7_run_meta.get("verifier_backend", "sahi"))
    ...
    for v in seq_list:
            ...
            if cost_writer is not None:
                try:
                    rec_cost = {
                        ...
                        "verifier_executed": int(sahi_on),
                        "num_verifier_regions": int(tiles_used) if float(tiles_used).is_integer() else float(tiles_used),
                        "sahi_on": int(sahi_on),
        ...
        row = {
            ...
            "verifier_backend": str(verifier_backend),
            "verifier_on_ratio": mean([float(x) for x in sahi_on_v]),
            "avg_verifier_regions": mean(tiles_v),
            ...
            "empty_verifier_on_ratio": float(empty_sahi_on_ratio_v),
            "empty_avg_verifier_regions": empty_avg_tiles_v,
            "pos_verifier_on_ratio": float(pos_sahi_on_ratio_v),
            "pos_avg_verifier_regions": pos_avg_tiles_v,
    ...
    sahi_on_ratio_mean = mean(sahi_on_all)
    if sahi_on_ratio_mean is None:
        sahi_on_ratio_mean = 0.0
    ...
    summary = {
        ...
        # Safety KPIs are computed robustly later (see "Step1: Safety KPI").
        # Do NOT reference per-video temporary variables here; for some datasets
        # (e.g., when --seq is missing/mismatched and the per-video loop is skipped),
        # those variables may never be assigned and would crash evaluation.
        "safety": {
        ...
        "sanity": {
            ...
            "verifier_backend": str(verifier_backend),
            "verifier_on_ratio": float(sahi_on_ratio_mean),
            "avg_verifier_regions_per_on_frame": float(total_tiles_sahi_all) / float(
                num_sahi_frames_all) if num_sahi_frames_all > 0 else 0.0,
            "sahi_on_ratio": float(sahi_on_ratio_mean),
        ...
        "cost": {
            "verifier_backend": str(verifier_backend),
            "verifier_on_ratio": float(sahi_on_ratio_mean),
            "avg_verifier_regions": {"mean": avg_tiles_mean, "p90": avg_tiles_p90},
            "sahi_on_ratio": float(sahi_on_ratio_mean),
            "avg_tiles_per_frame": {"mean": avg_tiles_mean, "p90": avg_tiles_p90},
            "latency_ms": {"mean": lat_mean, "p90": lat_p90},
            "fps": fps,
            "empty_wasted_compute": {
                "empty_verifier_on_ratio": float(empty_sahi_on_ratio_global),
                "empty_avg_verifier_regions": float(empty_avg_tiles_global),
                "empty_sahi_on_ratio": float(empty_sahi_on_ratio_global),
                "empty_avg_tiles": float(empty_avg_tiles_global),
            },
            "num_verifier_frames": int(num_sahi_frames_all),
            "total_verifier_regions": float(total_tiles_all),
            "avg_verifier_regions_per_on_frame": float(total_tiles_sahi_all) / float(
                num_sahi_frames_all) if num_sahi_frames_all > 0 else 0.0,
    ...
    # Step2: confirmation statistics from local-verifier policy log (optional)
    confirm_stats = {}
    try:
        import glob
    ...
    # Step3: dual-budget ledger statistics from local-verifier policy log (optional)
    budget_stats = {}
    try:
        import glob
        stats_dir = os.path.dirname(str(args.stats)) if args.stats is not None else str(args.out_dir)
    ...
    try:
        ...
        v7_log_path = None
        if isinstance(stats_run_meta, dict):
            v7_meta = stats_run_meta.get("local_verifier_policy")
            if not isinstance(v7_meta, dict):
                v7_meta = stats_run_meta.get("v7") if isinstance(stats_run_meta.get("v7"), dict) else None
            if isinstance(v7_meta, dict):
                v7_log_path = v7_meta.get("log_path")
        if isinstance(v7_log_path, str) and v7_log_path.strip() != "" and os.path.exists(v7_log_path):
            v7_logs = json.load(open(v7_log_path, "r"))
