# sahi_temporal/v6/tile_planner.py
...
@dataclass
class TilePlannerConfigV6:
    cand_extra: int = 6
    protect_neighbor_hops: int = 1


class TilePlannerV6:
    def __init__(self, cfg: Optional[TilePlannerConfigV6] = None) -> None:
        self.cfg = cfg if cfg is not None else TilePlannerConfigV6()
    ...
    def build(
        ...
        if is_full_burst:
            ...
            return TilePlan(tiles=all_ids, tiles_protect=[], tiles_explore=all_ids, K_protect=0, K_explore=len(all_ids), coverage_meta={'exp_cfg': exp_cfg, 'is_full_burst': True})
        if K <= 0:
            return TilePlan(tiles=[], tiles_protect=[], tiles_explore=[], K_protect=0, K_explore=0, coverage_meta={})
        ...
        score_mode = str(exp_cfg.get("score_mode", getattr(plan, "score_mode", "mean")))
        apply_row_quota = bool(exp_cfg.get("apply_row_quota", getattr(plan, "apply_row_quota", False)))
        quota_mode = str(exp_cfg.get("quota_mode", getattr(plan, "quota_mode", "top1")))
        use_memory = bool(exp_cfg.get("use_memory", getattr(plan, "use_memory", False)))
        memory_ttl = int(exp_cfg.get("memory_ttl", getattr(plan, "memory_ttl", 0)))
        ...
        # EvidenceBuilderV6 stores raw arrays in ev.raw.
        n_tiles = len(tiles)
